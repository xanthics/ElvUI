	...
	for i = 1, #auraTable do
		...
		if buffInfo.filter == "HARMFUL" then
			...
			if E.private.auras.mergeVanity and not expiring and C_VanityCollection.IsConsolidatedVanityBuff(buffInfo.spellID) then
				button:SetParent(ElvuiVanityBuffsContainer)
				button:Show()
			...
			elseif E.private.auras.mergeConsolidated and not expiring and buffInfo.shouldConsolidate then
				button:SetParent(ElvuiConsolidatedBuffsContainer)
				button:Show()
...
function A:ActuallyUpdateAllAnchors(buttons, container, tooltip)
	...
	for _, buff in pairs(buttons) do
		...
		local tick, cycle = floor((index - 1) % wrapAfter), floor((index - 1) / wrapAfter)
		buff:ClearAllPoints()
		buff:SetPoint("TOPLEFT", container, tick * xOffset, cycle * wrapYOffset)
		buff:SetSize(size, size)
	end
	tooltip:SetWidth(min(index * size, 4 * size))
	tooltip:SetHeight(floor((index + 3) / 4 + 1) * size)
end

function A:ElvuiConsolidatedBuffs_UpdateAllAnchors()
	self:ActuallyUpdateAllAnchors(consolidatedButton, ElvuiConsolidatedBuffsContainer, ElvuiConsolidatedBuffsTooltip)
end

function A:ElvuiVanityBuffs_UpdateAllAnchors()
	self:ActuallyUpdateAllAnchors(vanityButton, ElvuiVanityBuffsContainer, ElvuiVanityBuffsTooltip)
end
